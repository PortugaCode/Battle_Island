...
public class Test_select : MonoBehaviour
{
    ...
    public Text newItemName;	// 새로 입력된 플레이어의 닉네임
    ...
    void Start()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            if (File.Exists(Test_Json.instance.path + $"{i}"))	// 데이터가 있는 경우
            {
                savefile[i] = true;			// 해당 슬롯 번호의 bool배열 true로 변환
                Test_Json.instance.nowSlot = i;	// 선택한 슬롯 번호 저장
                Test_Json.instance.LoadData();	// 해당 슬롯 데이터 불러옴
                slotText[i].text = Test_Json.instance.nowItem.itemName;	// 버튼에 닉네임 표시
            }
            else	// 데이터가 없는 경우
            {
    ...
    {
        Test_Json.instance.nowSlot = number;	// 슬롯의 번호를 슬롯번호로 입력함.

        if (savefile[number])	// bool 배열에서 현재 슬롯번호가 true라면 = 데이터 존재한다는 뜻
        {
            Test_Json.instance.LoadData();	// 데이터를 로드하고
            GoGame();	// 게임씬으로 이동
        }
    ...
    {
        if (!savefile[Test_Json.instance.nowSlot])	// 현재 슬롯번호의 데이터가 없다면
        {
            Test_Json.instance.nowItem.itemName = newItemName.text; // 입력한 이름을 복사해옴
            Test_Json.instance.SaveData(); // 현재 정보를 저장함.
        }
